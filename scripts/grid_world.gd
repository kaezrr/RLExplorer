...
func generate_map(map_seed: int) -> Array:
	...
	print("WARNING: Could not generate a valid map after ", MAX_GENERATION_ATTEMPTS, " attempts.")
...
func generate_raw_map(map_seed: int) -> Array:
	...
	# --------------------------------------------------
	var collectible_count := rng.randi_range(MIN_COLLECTIBLES, MAX_COLLECTIBLES)
