...
func get_reachable_cells(grid_data: Array, start_pos: Vector2i) -> Array[Vector2i]:
...
func is_map_valid(grid_data: Array, start_pos: Vector2i) -> bool:
	var reachable := get_reachable_cells(grid_data, start_pos)
...
func is_inside_grid(grid_data: Array, position: Vector2i) -> bool:
	if position.y < 0:
		return false
