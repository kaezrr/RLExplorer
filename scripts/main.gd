...
func load_configured_q_table() -> bool:
	...
	var loaded := q_learning.load_q_table(
		Q_TABLE_PATH
	)

	print(
		"Q-table load successful: ",
		loaded
	)
...
func run_configured_training() -> void:
	...
	if training_in_progress:
		...
		print(
			"Training already in progress, ignoring request."
		)
	...
	if playback_running:
		...
		print(
			"Cannot start training while playback is running."
		)

		print(
			"Press M to return to manual mode, then try again."
		)
...
func start_trained_playback() -> void:
	...
	if training_in_progress:
		...
		print(
			"Cannot start playback while training is in progress."
		)
	...
	if not load_configured_q_table():
		...
		print(
			"ERROR: Could not load trained Q-table."
		)
